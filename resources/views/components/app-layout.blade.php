...
<body class="h-full antialiased" x-data="appShell()" :class="{ 'overflow-hidden': mobileOpen }">
...
<div class="app-shell">
            ...
            <div class="flex items-center gap-3">
                <div class="relative flex h-10 w-10 items-center justify-center rounded-2xl bg-emerald-400/15 text-emerald-200">
                    ...
                    <svg class="relative w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linecap="round" stroke-width="2" d="M9 12l2 2 4-4m5.586-3.414A2 2 0 0019.172 5H4.828a2 2 0 00-1.414.586l-1.414 1.414A2 2 0 001 8.414V10a2 2 0 002 2h18a2 2 0 002-2V8.414a2 2 0 00-.586-1.414l-1.414-1.414z"/>
                    </svg>
                </div>
                <div>
        ...
        <div class="hidden md:flex items-center justify-end px-3 py-3 border-t border-white/10">
            <button @click="sidebarCollapsed = !sidebarCollapsed"
                ...
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 transition-transform duration-300"
                    ...
                    <path stroke-linecap="round" stroke-linecap="round" d="M11 19l-7-7 7-7m8 14l-7-7 7-7"/>
                </svg>
            </button>
        </div>
    </aside>
    ...
    <header class="topbar"
            :style="!$isTekpol ? (sidebarCollapsed ? 'left:104px' : 'left:296px') : 'left:0'"
            style="@if($isTekpol) left:0; @else left:296px; @endif">
        <div class="flex items-center gap-3 min-w-0">
            @if(!$isTekpol)
            <button @click="mobileOpen = !mobileOpen" class="md:hidden btn-icon">
            ...
            @endif
            <div class="min-w-0">
                <p class="hidden text-[11px] uppercase tracking-[0.2em] text-slate-400 sm:block">MOSIP Workspace</p>
                <h1 class="truncate text-base font-semibold text-slate-800 leading-tight">{{ $title ?? 'Dashboard' }}</h1>
                <p class="hidden truncate text-xs text-slate-400 sm:block">{{ $subtitle ?? 'MOSIP – Monitoring Operasional Sipil & Perumahan' }}</p>
            </div>
        </div>

        <div class="flex items-center gap-2">
            <div class="relative hidden sm:block">
                <input type="text" placeholder="Cari..." class="form-input !py-2.5 !pl-9 !pr-4 w-44 lg:w-64 text-xs !bg-white/80">
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open"
                    ...
                    <div class="w-9 h-9 rounded-2xl bg-gradient-to-br from-primary-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold shadow-sm">
                        @auth
                    ...
                    <svg class="w-4 h-4 text-slate-400 transition-transform duration-200" :class="open ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div x-show="open" x-cloak @click.outside="open = false"
                     ...
                     class="absolute right-0 top-full mt-2 w-52 overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-2xl z-50">
                    ...
                    <div class="py-1">
                        <a href="#" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition-colors">
                            <svg class="w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linecap="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                            Profil Saya
                        </a>
                        <a href="#" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition-colors">
                            <svg class="w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linecap="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 001.066-2.573c-1.543.94-3.31-.826-2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linecap="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                            Pengaturan
                        </a>
                    </div>
                    <div class="border-t border-slate-100 py-1">
                        <form method="POST" action="{{ route('logout') }}">
                            ...
                            <button type="submit" class="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 transition-colors">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linecap="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/></svg>
                                Keluar
                            </button>
                        </form>
                    </div>
    ...
    <main id="main-content" class="page-content"
          :class="{ 'sidebar-collapsed': sidebarCollapsed && !$isTekpol }"
          style="@if($isTekpol) margin-left: 0; @endif">
